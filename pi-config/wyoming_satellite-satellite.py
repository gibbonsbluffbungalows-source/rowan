...
class WakeStreamingSatellite(SatelliteBase):
    ...
    # Follow-up window: after a QUESTION reply, listen this long for the
    # guest to answer without the wake word. SETTLE drains the playback tail.
    FOLLOWUP_SECONDS = 15.0
    FOLLOWUP_SETTLE = 1.0
    ...
    async def event_from_server(self, event: Event) -> None:
        ...
        if VoiceStarted.is_type(event.type) and self._followup_active:
            # Guest started talking inside the follow-up window: keep the
            # stream open and cancel the silence timeout.
            self._followup_active = False
        ...
        elif AudioStop.is_type(event.type) and not self._is_paused:
            # Reply finished. Open a follow-up window ONLY if the reply was a
            # question to the guest; a decisive statement leaves the mic shut
            # so the guests can talk it over without Rowan chiming in.
            if self._reply_invites_followup():
                self._cancel_followup()
                self._followup_task = asyncio.create_task(self._followup_listen())
            self._last_reply_text = ""

    async def trigger_synthesize(self, synthesize: Synthesize) -> None:
        # Remember reply text so AudioStop can tell question vs statement.
        self._last_reply_text = synthesize.text or ""
        await super().trigger_synthesize(synthesize)

    def _reply_invites_followup(self) -> bool:
        import re
        text = self._last_reply_text or ""
        # Drop trailing control tags like [KB_GAP: ...] the guest never hears.
    ...
    def _cancel_followup(self) -> None:
        self._followup_active = False
        if (self._followup_task is not None) and (not self._followup_task.done()):
            self._followup_task.cancel()
        self._followup_task = None

    async def _followup_listen(self) -> None:
        try:
            # Let the playback tail drain so VAD does not trigger on Rowan.
            await asyncio.sleep(self.FOLLOWUP_SETTLE)
            if self._is_paused or self.microphone_muted or self.is_streaming:
                return
            self._followup_active = True
            self.is_streaming = True
            _LOGGER.info("Follow-up window open (%.1fs) after question", self.FOLLOWUP_SECONDS)
            await self.trigger_streaming_start()
            await self._send_run_pipeline()
            await asyncio.sleep(self.FOLLOWUP_SECONDS)
            if self._followup_active and self.is_streaming:
                # Silence: return to wake word detection.
                self.is_streaming = False
                await self.trigger_streaming_stop()
                if not self._is_paused:
                    await self._send_wake_detect()
                    _LOGGER.info("Follow-up timed out; waiting for wake word")
        except asyncio.CancelledError:
            pass
        finally:
            self._followup_active = False
